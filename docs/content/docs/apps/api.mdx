...
In this repo, `apps/api` owns API-only runtime entrypoints such as health checks, cron handlers, and inbound webhooks.
Keep routes thin and idempotent, and move reusable provider or domain behavior into `packages/*`.
...
export const GET = async () => {
  const users = await database.select().from(schema.appUser);
