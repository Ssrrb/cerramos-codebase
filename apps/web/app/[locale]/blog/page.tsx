...
const renderPosts = (
  posts: Awaited<ReturnType<typeof blog.getPosts>>,
  emptyMessage: string
) => {
  if (!posts.length) {
    return (
...
const BlogIndex = async ({ params }: BlogProps) => {
  const { locale } = await params;
  const dictionary = await getDictionary(locale);
  const posts = isCMSConfigured ? null : await blog.getPosts();
        ...
        <div className="container mx-auto flex flex-col gap-14">
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            {isCMSConfigured ? (
              ...
              renderPosts(posts ?? [], dictionary.web.blog.meta.description)
            )}
          </div>
        </div>
      </div>
