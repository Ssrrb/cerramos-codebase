...
In this repo, `@repo/analytics` centralizes shared analytics provider wiring and helpers.
Keep feature-specific event semantics close to the owning product flow unless they become shared conventions.
...
To automatically capture authentication and payment events, combine PostHog's server-side library with Better Auth callbacks or route handlers in the `app` app and Stripe webhooks in the `api` app:

<Mermaid chart={`
graph TD
  A[User Action in App] -->|Triggers| B[Better Auth Handler]
  A -->|Triggers| E[Payments Webhook]
  A -->|Client-Side Call| PostHog
  B -->|Sends Data| C1[api/auth]
