...
They are then attached to the response within the proxy layer in `apps/app/proxy.ts` and `apps/web/proxy.ts`. Adjusting the configuration in these files will only affect the specific app.
