...
In this repo, `@repo/notifications` owns shared notification plumbing and UI helpers.
Notifications are not the source of truth for operational state; triggering policy belongs with the owning app or domain flow.
