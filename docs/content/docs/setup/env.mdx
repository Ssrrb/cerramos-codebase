...
Required for first-party authentication and shared sessions across `app` and `web`.
...
<Step>
  Generate a long secret for `BETTER_AUTH_SECRET`
</Step>

<Step>
  Set `BETTER_AUTH_URL` to the `app` origin that hosts `/api/auth/[...all]`
</Step>

<Step>
  Create Google OAuth credentials and add the client ID and secret if you want Google sign-in
</Step>
...
Add this in production if `app` and `web` run on subdomains of the same parent domain:
