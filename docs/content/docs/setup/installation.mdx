...
You will need to scaffold the database using the schema defined in `packages/database/schema.ts`:

```package-install
npm run migrate
```

For more details on the default Drizzle configuration (using Neon), refer to the [Database Configuration Guide](https://www.next-forge.com/packages/database#default-configuration).

## CMS

You will need to setup the CMS. Follow the instructions [here](/docs/packages/cms), but the summary is:
