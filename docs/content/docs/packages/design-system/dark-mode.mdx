...
The `next-themes` provider is already configured in the application, handling theme persistence and system preference detection automatically. Shared UI such as Sonner and the first-party auth screens can respect the same theme tokens and dark-mode state.
